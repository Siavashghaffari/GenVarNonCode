...
def ADD_annotations (df, Overlap_probands, Overlap_unaffected, Overlap_parents, case):
    ...
    CNV_MSSNG = Pd.read_csv("/hpf/largeprojects/tcagstor/users/sghaffari/SVs.MSSNG.freq_1percent.HQR_Small.tsv", sep='\t', low_memory=False)
    #MSSNG = Pd.read_csv("/hpf/largeprojects/tcagstor/users/sghaffari/MSSNG_metadata.tsv", sep='\t')
    CNV_SSC = Pd.read_csv("/hpf/largeprojects/tcagstor/users/sghaffari/SVs.SSC.freq_1percent.HQR_Small.tsv", sep='\t', low_memory=False)
    ...
    CNV = Pd.concat([MSSNG_2,SSC_2])
    CNV = CNV.rename(columns={"start":"START","end":'END',"chrm":"CHROM","sv_type":"SVTYPE"})
    ...
    DF_FINAL.to_csv("Final_ASD_overlaps_info_SV"+case+".tsv", sep="\t")
    ...
    FAM_7 = FAM_5.rename(columns={"Same_Family":"Family ID"})
    
    FAM_8 = (
    FAM_FINAL.merge(FAM_7, 
              on=["lncipediaGeneID","SVTYPE", "Family ID"],
              how='inner'))
    
    FAM_9 = FAM_8.groupby(["lncipediaGeneID","SVTYPE", "Family ID"]).agg({"Relation":lambda x: x.tolist(),
                                                          "Affection_binary": lambda x: x.tolist(),"Sex_binary":lambda x: x.tolist()}).reset_index()
    
    FAM_10 = FAM_9.groupby(["lncipediaGeneID","SVTYPE"]).agg({"Relation":lambda x: x.tolist(),"Affection_binary": lambda x: x.tolist(),
                                                              "Sex_binary":lambda x: x.tolist()}).reset_index()
    
    FAM_11 = FAM_10.rename(columns={"Relation":"Relantion_F","Affection_binary":"Affection_F","Sex_binary":"Sex_F"})
    
    #FAM_12 = FAM_11[["lncipediaGeneID","SVTYPE","Relation_F", "Affection_F", "Sex_F"]]
    ...
    #genomAD_coverage_freq

    # Replace "-" with 0 to simplfy the analysis later
    df_Final_test["gnomAD_rareSV"] = df_Final_test["gnomAD_rareSV"].replace("-",0)
    df_Final_test["gnomAD_commonSV"] = df_Final_test["gnomAD_commonSV"].replace("-",0)   
    df_Final_test["gnomAD_rareSV"] = Pd.to_numeric(df_Final_test["gnomAD_rareSV"], downcast="float")
    df_Final_test["gnomAD_commonSV"] = Pd.to_numeric(df_Final_test["gnomAD_commonSV"], downcast="float")

    DGV1 = df_Final_test.groupby(["lncRNA_gene","SVTYPE"])["gnomAD_rareSV"].mean()
    DGV_freq1 = DGV1.to_frame().reset_index()
    DGV_freq1 = DGV_freq1.rename(columns={"lncRNA_gene":"lncipediaGeneID","gnomAD_rareSV":"gnomAD_rare_Freq"})
    #genomAD common SVs
    DGV2 = df_Final_test.groupby(["lncRNA_gene","SVTYPE"])["gnomAD_commonSV"].mean()
    DGV_freq2 = DGV2.to_frame().reset_index()
    DGV_freq2 = DGV_freq2.rename(columns={"lncRNA_gene":"lncipediaGeneID","gnomAD_commonSV":"gnomAD_common_Freq"})
    ...
    df_DGV_test = (
    df_DGV_test.merge(FAM_11, 
              on=["lncipediaGeneID","SVTYPE"],
              how='left'))
    
    
    #df_DGV_test = df_DGV_test[["lncipediaGeneID","Exon_no","SVTYPE","DGV Freq"]]
    df_DGV_test["gnomAD_rare_Freq"] = df_DGV_test["gnomAD_rare_Freq"].fillna(0)
    df_DGV_test["gnomAD_common_Freq"] = df_DGV_test["gnomAD_common_Freq"].fillna(0)
